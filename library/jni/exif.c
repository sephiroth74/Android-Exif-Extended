...
//				{ // We already have a jpeg comment.
...
//				if (ByteCount > 1)
//				{
//					if (ByteCount > MAX_COMMENT_SIZE)
//						ByteCount = MAX_COMMENT_SIZE;
...
//					// Already have a comment (probably windows comment), skip this one.
//					if (ShowTags)
//						printf("Multiple comments in exif header\n");
//					break; // Already have a windows comment, skip this one.
//				}
//
...
//					if (msiz > ByteCount)
//						msiz = ByteCount;
//					if (msiz > MAX_COMMENT_SIZE - 1)
//						msiz = MAX_COMMENT_SIZE - 1;
...
//				}
			...
			case TAG_EXPOSURE_INDEX:
				if (ImageInfo.ISOSpeedRatings == 0)
				{
					...
					LOGD("ISOSpeedRatings: %i", ImageInfo.ISOSpeedRatings);
				}
				break;
		...
		if (DIR_ENTRY_ADDR(DirStart, NumDirEntries) + 4 <= OffsetBase + ExifLength)
		{
			...
			if (Offset)
			{
				...
				} else
				{
					...
					{
						if (ShowTags)
							printf("%s    Continued directory ", IndentString);
						ProcessExifDir(SubdirStart, OffsetBase, ExifLength, NestingLevel + 1);
					}
				}
	...
	if (ThumbnailOffset)
	{
		...
		if (ThumbnailOffset <= ExifLength)
		{
			if (ThumbnailSize > ExifLength - ThumbnailOffset)
			{
				...
				if (ShowTags)
					printf("Thumbnail incorrectly placed in header\n");
...
void ShowImageInfo(int ShowFileInfo)
{
	...
	{
		if (ImageInfo.Flash & 1)
		{
			LOGD("Flash used   : Yes");
			switch (ImageInfo.Flash)
			{
				case 0x5:
					LOGD(" (Strobe light not detected)");
					break;
				case 0x7:
					LOGD(" (Strobe light detected) ");
					break;
				case 0x9:
					LOGD(" (manual)");
					break;
				case 0xd:
					LOGD(" (manual, return light not detected)");
					break;
				case 0xf:
					LOGD(" (manual, return light  detected)");
					break;
				case 0x19:
					LOGD(" (auto)");
					break;
				case 0x1d:
					LOGD(" (auto, return light not detected)");
					break;
				case 0x1f:
					LOGD(" (auto, return light detected)");
					break;
				case 0x41:
					LOGD(" (red eye reduction mode)");
					break;
				case 0x45:
					LOGD(" (red eye reduction mode return light not detected)");
					break;
				case 0x47:
					LOGD(" (red eye reduction mode return light  detected)");
					break;
				case 0x49:
					LOGD(" (manual, red eye reduction mode)");
					break;
				case 0x4d:
					LOGD(" (manual, red eye reduction mode, return light not detected)");
					break;
				case 0x4f:
					LOGD(" (red eye reduction mode, return light detected)");
					break;
				case 0x59:
					LOGD(" (auto, red eye reduction mode)");
					break;
				case 0x5d:
					LOGD(" (auto, red eye reduction mode, return light not detected)");
					break;
				case 0x5f:
					LOGD(" (auto, red eye reduction mode, return light detected)");
					break;
			}
		} else
		{
			LOGD("Flash used   : No (%i)", ImageInfo.Flash);
			switch (ImageInfo.Flash)
			{
				case 0x18:
					LOGD(" (auto)");
	...
	{
		LOGD("Focal length : %4.1fmm", (double)ImageInfo.FocalLength);
		if (ImageInfo.FocalLengthIn35mmFilm)
		{
			LOGD("  (35mm equivalent: %dmm)", ImageInfo.FocalLengthIn35mmFilm);
		}
		printf("\n");
	}
	...
	if (ImageInfo.FocalPlaneXResolution != 0 && ImageInfo.PixelXDimension > 0)
	{
		...
		float CCDWidth = (float) (ImageInfo.PixelXDimension * unit / ImageInfo.FocalPlaneXResolution);
		LOGD("PixelXDimension: %i", ImageInfo.PixelXDimension);
		LOGD("FocalPlaneXResolution: %.4f", ImageInfo.FocalPlaneXResolution);
		LOGD("FocalPlaneResolutionUnit: %i", ImageInfo.FocalPlaneResolutionUnit);
		LOGD("unit: %.4f", unit);
		LOGD("CCD width    : %4.2fmm\n", CCDWidth);
		...
		if (ImageInfo.FocalLength && ImageInfo.FocalLengthIn35mmFilm == 0)
		{
			...
			int focal35mm = (int) (ImageInfo.FocalLength / CCDWidth * 36 + 0.5);
	...
	if (ImageInfo.ExposureTime)
	{
		...
		{
			LOGD("Exposure time: %6.4f s ", (double)ImageInfo.ExposureTime);
		} else
		{
			LOGD("Exposure time: %5.3f s ", (double)ImageInfo.ExposureTime);
		}
		if (ImageInfo.ExposureTime <= 0.5)
		{
			LOGD(" (1/%d)", (int)(0.5 + 1/ImageInfo.ExposureTime));
		}
		printf("\n");
	}
	if (ImageInfo.FNumber)
	{
		LOGD("Aperture     : f/%3.1f\n", (double)ImageInfo.FNumber);
	}
	if (ImageInfo.SubjectDistance)
	{
		if (ImageInfo.SubjectDistance < 0)
		{
			LOGD("Focus dist.  : Infinite\n");
		} else
		{
			LOGD("Focus dist.  : %4.2fm\n", (double)ImageInfo.SubjectDistance);
		}
	}

	if (ImageInfo.ISOSpeedRatings)
	{
		LOGD("ISO equiv.   : %2d\n", (int)ImageInfo.ISOSpeedRatings);
	}
	...
	if (ImageInfo.ExposureBiasValue)
	{
		...
		LOGD("Exposure bias: %4.2f", (double)ImageInfo.ExposureBiasValue);
	}

	switch (ImageInfo.Whitebalance)
	{
		case 1:
			LOGD("Whitebalance : Manual\n");
			break;
		case 0:
			LOGD("Whitebalance : Auto\n");
			break;
	}
	...
	{
		case 1:
			LOGD("Light Source : Daylight\n");
			break;
		case 2:
			LOGD("Light Source : Fluorescent\n");
			break;
		case 3:
			LOGD("Light Source : Incandescent\n");
			break;
		case 4:
			LOGD("Light Source : Flash\n");
			break;
		case 9:
			LOGD("Light Source : Fine weather\n");
			break;
		case 11:
			LOGD("Light Source : Shade\n");
			break;
		default:
	...
	{ // 05-jan-2001 vcs
		...
		{
			case 1:
				LOGD("Exposure     : Manual\n");
				break;
			case 2:
				LOGD("Exposure     : program (auto)\n");
				break;
			case 3:
				LOGD("Exposure     : aperture priority (semi-auto)\n");
				break;
			case 4:
				LOGD("Exposure     : shutter priority (semi-auto)\n");
				break;
			case 5:
				LOGD("Exposure     : Creative Program (based towards depth of field)\n");
				break;
			case 6:
				LOGD("Exposure     : Action program (based towards fast shutter speed)\n");
				break;
			case 7:
				LOGD("Exposure     : Portrait Mode\n");
				break;
			case 8:
				LOGD("Exposure     : LandscapeMode \n");
				break;
			default:
				break;
		}
	...
	switch (ImageInfo.ExposureMode)
	{
		case 0: // Automatic (not worth cluttering up output for)
			break;
		case 1:
			LOGD("Exposure Mode: Manual\n");
			break;
		case 2:
			LOGD("Exposure Mode: Auto bracketing\n");
			break;
	}
	...
	{
		LOGD("Focus range  : ");
		switch (ImageInfo.SubjectDistanceRange)
		{
			case 1:
				LOGD("macro");
				break;
...
void ShowConciseImageInfo(void)
{
	...
	if (ImageInfo.FNumber)
	{
		printf(" f/%3.1f", (double) ImageInfo.FNumber);
	}
