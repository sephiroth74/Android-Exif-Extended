    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        if (Tag == 1 && Components > 16){
            ...
            if (IsoCode >= 16 && IsoCode <= 24){
                ImageInfo.ISOSpeedRatings = 50 << (IsoCode-16);
            } 
        }
        ...
        if (Tag == 4 && Format == FMT_USHORT){
            ...
            if (Components > 19 && ImageInfo.SubjectDistance <= 0) {
                // Indicates the distance the autofocus camera is focused to.
                // Tends to be less accurate as distance increases.
                int temp_dist = Get16u(ValuePtr + 19*sizeof(unsigned short));
                if (temp_dist != 65535){
                    ImageInfo.SubjectDistance = (float)temp_dist/100;
                }else{
                    ImageInfo.SubjectDistance = -1 /* infinity */;
                }
            }
        }
    }
